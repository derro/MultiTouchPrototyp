...
class SettingsViewController: UIViewController, AVSpeechSynthesizerDelegate, UIGestureRecognizerDelegate {
    ...
    func handlePinch(gesture: UIPinchGestureRecognizer) {
        if(gesture.scale > 2.0) {
            ...
            {
                //Pinch open detected
                speak("Verlasse Einstellungen. Gehe zum Start (Geste Pinch)")
                dismissViewControllerAnimated(false, completion: nil)
                //performSegueWithIdentifier("goToStartView", sender: nil)
            }
